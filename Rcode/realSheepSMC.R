...
# Remove all dead sheep and order by year:
...
# Number of sheep that have been detected (not always with weight measured!!!)
...
# Number of sheep that have their size measured (and have not died)
...
# Make a plot to compare the alive sheep against the total detected population.
# sheep counts:
...
# Interpolate the data points over a larger number of elements.
# plot the fitted dist over the data of the observed capture probs:
x <-  seq(0, 1, l=3000)
...
skeleton = list(
  # Variables with two dimensions have the form p0+p1*x
  survPars = rep(NA, 2),
  # Variables with three dimensions have the form N(mu0+mu1*x,sigma²)
  growthPars = rep(NA, 3),
  reprPars = rep(NA, 2),
  # Variables with one dimension have the form lambda + x
  offNumPars = NA,
...
startValues %<>% unlist
logTargetIPM(startValues, logTargetPars = IPMLTP, returnNeg = T, printProp = T)
...
############################## RUN THE PMCMC ###################################
IPMLTP$b <- 100
sheepStart <- readRDS("../RDSobjects/sheepChain4")[[1]][10000,-1]

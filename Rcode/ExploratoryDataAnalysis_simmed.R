...
age<-simmedData%>%group_by(individual)%>%summarise(birth.yr=min(census.number))
simmedData%<>%merge(age)
simmedData$age<-simmedData$census.number-simmedData$birth.yr
max.cens <- simmedData$census.number %>% max
simmedData%<>%filter(census.number>=max(c((max.cens-12L),1L)))

simmedData$sheep.yr<-simmedData$census.number-min(simmedData$census.number,na.rm = T)
simmedData$rep<-factor(simmedData$reproduced); simmedData$rep[is.na(simmedData$rep)]<-0

simmedData%>%group_by(sheep.yr)%>%rstatix::get_summary_stats()
simmedData%>%rstatix::kruskal_test(size~sheep.yr)

# Weight distribution evolution per census year
p<-ggplot(simmedData,aes(exp(size)))+geom_histogram(aes(y=..density..),fill="blue",alpha=0.2)+
  geom_density() +labs(x="Size [kg]",y="Density",title="Time Series")  +theme(plot.title = element_text(hjust = 0.5)) + facet_grid(sheep.yr~.)
ggsave("TimeSeriesDensityHist.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 8,height = 7)
# Growth:
p<-ggplot(simmedData,aes(exp(prev.size),exp(size)))+stat_density2d_filled(contour_var = "ndensity") + geom_abline(slope=1) + xlim(c(10,30)) + ylim(c(10,30)) +
...
ggsave("GrowthDensity2D.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 4)
# Size of Offspring
p<-ggplot(simmedData,aes(exp(parent.size),exp(size)))+stat_density2d_filled(contour_var = "ndensity") + geom_abline(slope=1) + ylim(c(5,20)) + xlim(c(10,30)) +
...
ggsave("OffspringSizeDensity2D.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 4)
# Number of Offspring
simmedData$num.born<-simmedData$off.born ; simmedData$num.born[simmedData$num.born<1] <- NA ; simmedData$num.born%<>%factor()
p<-ggplot(filter(simmedData,!is.na(num.born)),aes(num.born))+geom_bar(fill="darkorchid3",na.rm = T) + labs(x="Number of Offspring",y="Count")
ggsave("NumOffspring.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 5,height = 4)
# Survival Probability
p<-ggplot(filter(simmedData,!is.na(survived)),aes(factor(survived),exp(prev.size)))+geom_violin(aes(fill=factor(survived)),na.rm = T) + 
  labs(x="Survived",y="Size at Previous Census [kg]")+ scale_fill_discrete(name = "Survived")
ggsave("SurvivalViolin.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 4)
# Reproduction Probability
simmedData$rep<-factor(simmedData$reproduced); simmedData$rep[is.na(simmedData$rep)]<-0
p<-ggplot(simmedData,aes(rep,exp(size)))+geom_violin(aes(fill=rep)) + 
  labs(x="Reproduced",y="Size at Current Census [kg]") + scale_fill_discrete(name = "Reproduced")
ggsave("ReproducedViolin.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 4)
...
# Grid
mylexis <- lexis_grid(year_start = min(simmedData$sheep.yr,na.rm = T), year_end = max(simmedData$sheep.yr,na.rm = T)+2, age_start = 1, age_end = 20)
...
alivedata<-simmedData%>%group_by(age,sheep.yr)%>%summarise(total=as.integer(sum(survived,na.rm = T)))%>%drop_na()%>%as.data.frame()%>%PrepLexisData
# Calculate number of deaths per age group, per year
survdata<-simmedData%>%group_by(age,sheep.yr)%>%summarise(total=as.integer(sum(abs(survived-1),na.rm = T)))%>%drop_na()%>%as.data.frame()%>%PrepLexisData
# Calculate number of births occurred, per age group, per year
reprdata<-simmedData%>%group_by(age,sheep.yr)%>%summarise(total=sum(as.integer(rep),na.rm = T))%>%drop_na()%>%as.data.frame()%>%PrepLexisData
...
ggsave("AliveLexis.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 6) 
...
ggsave("AliveLexisColorbar.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 6) 
# Survival
...
ggsave("SurvLexis.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 6) 
p<-plot_discrete_cbar((unique(as.integer(exp((hist(log(survdata$total+1),breaks = 10,plot = F))$breaks)))-1), 
                   spacing = "constant", palette="Spectral",legend_direction = "vertical",direction = -1)
ggsave("SurvLexisColorbar.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 6) 
# Reproduction
...
ggsave("ReprodLexis.png", plot=p,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 6)
q<-plot_discrete_cbar((unique(as.integer(exp((hist(log(reprdata$total+1),breaks = 10,plot = F))$breaks)))-1), 
                      spacing = "constant", palette="Spectral",legend_direction = "vertical",direction = -1)
ggsave("ReprLexisColorbar.png", plot=q,path = paste0(directory,'Plots/Hamish/simmedData'),width = 6,height = 6) 
